...
brigadachat_ORIG_GROUP="im.brigadachat"
brigadachat_ORIG_TEAM="4NXF6Z997G"
#brigadachat_NEW_GROUP="com.example.brigadachat"
#brigadachat_NEW_TEAM="ABCDE12345"
...
# export brigadachat_INSTALL_IPA=1

### Rotate IDs ###
[ -n "${brigadachat_NEW_GROUP}" ] && {
	# App group IDs
	sed -i "" "s/group.${brigadachat_ORIG_GROUP}.app/group.${brigadachat_NEW_GROUP}.app/g" "ios/brigadachat Share/brigadachat Share.entitlements"
	sed -i "" "s/group.${brigadachat_ORIG_GROUP}.app/group.${brigadachat_NEW_GROUP}.app/g" "ios/Runner/Runner.entitlements"
	sed -i "" "s/group.${brigadachat_ORIG_GROUP}.app/group.${brigadachat_NEW_GROUP}.app/g" "ios/Runner.xcodeproj/project.pbxproj"
	# Bundle identifiers
	sed -i "" "s/${brigadachat_ORIG_GROUP}.app/${brigadachat_NEW_GROUP}.app/g" "ios/Runner.xcodeproj/project.pbxproj"
}

[ -n "${brigadachat_NEW_TEAM}" ] && {
	# Code signing team
	sed -i "" "s/${brigadachat_ORIG_TEAM}/${brigadachat_NEW_TEAM}/g" "ios/Runner.xcodeproj/project.pbxproj"
}
cat << EOHELP
...
2. Fix it yourself
    ...
    - Ensure that Xcode created the App ID successfully (for brigadachat.app and brigadachat.app.brigadachat-Share)
...
[ -n "${brigadachat_INSTALL_IPA}" ] && {
  ...
  cfgutil --foreach install-app "${TMPDIR}/brigadachat.ipa"
  rm -rf "${TMPDIR}"
}
